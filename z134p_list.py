...
zlist = ["cat", "dog", "fox", "monkey", "mouse", "panda", "frog", "snake", "wolf"]
n = 1                   # 문제 번호
print("[타자 게임] 준비되면 엔터!")
input()                 # 사용자가 엔터를 누를 때까지 기다립니다.
start = time.time()     # 시작 시간을 기록합니다.

# ..random.choice(LIST) : 리스트에 든 자료들 중 임의로 하나를 고름.
q = random.choice(zlist)    # 단어 리스트에서 아무거나 하나 뽑습니다.
...
while n <= 5:           # 문제를 5번 반복합니다.
    ...
    if q == x:          # 문제와 입력이 같을 때(올바로 입력했을 때)
        ...
        q = random.choice(zlist)   # 새 문제를 뽑습니다.
    else:
        print("오타! 다시도전!")
